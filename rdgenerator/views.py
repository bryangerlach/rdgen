    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                iconimg = Image.open(iconfile.file.read())
                if iconimg.size[0] > 512:
                    wpercent = (512 / float(iconimg.size[0]))
                    hsize = int((float(iconimg.size[1]) * float(wpercent)))
                    iconimg = iconimg.resize((512, hsize), Image.Resampling.LANCZOS)

                iconbase64 = base64.b64encode(iconimg)
            except:
            ...
            try:
                ...
                logoimg = Image.open(logofile.file.read())
                if logoimg.size[0] > 512:
                    wpercent = (512 / float(logoimg.size[0]))
                    hsize = int((float(logoimg.size[1]) * float(wpercent)))
                    logoimg = logoimg.resize((512, hsize), Image.Resampling.LANCZOS)
                logobase64 = base64.b64encode(logoimg)
            except:
                logobase64 = b""
